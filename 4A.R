...
dir()
data4A <- read.csv("4A.csv")
names(data4A)[1]<-paste("species") 
names(data4A)[2]<-paste("SDE")
str(data4A)

#each graph has its own scale
p1 <- ggplot(data=data4A, aes(SDE))+geom_density()+facet_wrap(~species,scales="free")

#each graph has same scale. adjust sets smoothing of the variation, also plots are coloured  in
p2 <- ggplot(data=data4A, aes(SDE))+geom_density(fill="blue",colour=NA,alpha=.2,adjust=3)+geom_line(stat="density",adjust=3)+facet_wrap(~species,scales="fixed")

#all variations overlaid 
p3 <- ggplot(data=data4A, aes(SDE,colour=species))+geom_density(adjust=3) 

#all overlapping variations with ALL VARIATION superimposed. all variation is smoothed like the rest
p4 <- ggplot(data=data4A) +geom_density(aes(SDE,colour=species),adjust=3) + geom_density(aes(SDE),size=2,adjust=3) +ggtitle(str_wrap("Seed Dispersal Effectiveness for Chamaerops humilis (4A)",width=42))

bp1 <- ggplot(data=data4A)+geom_boxplot(aes(SDE,species)) +geom_boxplot(aes(SDE),size=2,alpha=0.2)+ylab('SPECIES')+ggtitle("Seed Dispersal of Chamaerops humilis (4A)")
#display plots
p1
p2
p3
...
summary_table <- data4A %>% group_by(species) %>% summarize(n=n(), mean_SDE <- mean(SDE, na.rm=TRUE), sd_SDE <- sd(SDE, na.rm=TRUE), se_error <- sd_SDE/sqrt(n))
view(summary_table)

species_list <- unique(data4A$species)
species_list

i = 1

#initialize empty vectors to do CV equality tests
SD_vector <-vector(mode='numeric',length=length(unique(data4A$species)))
mean_vector <-vector(mode='numeric',length=length(unique(data4A$species)))
n_vector <-vector(mode='numeric',length=length(unique(data4A$species)))

#make a vectors for SD, mean, and n of each species group (to calculate one against all variances)
for (i in 1:(length(unique(data4A$species))))
{
  #vector with standard deviation values for each species
  tmp<-subset(data4A,species==species_list[[i]])
...
SD_total <- sd(data4A$SDE, na.rm=TRUE)
mean_total <-mean(data4A$SDE)
n_total <-length(data4A$SDE)
...
x_vector<-as.vector(data4A$SDE)
grouping_vector<-as.vector(data4A$species)
...
s_tmp[[2]] <-SD_total
x_tmp[[2]] <-mean_total
one.v.all <-vector(mode='list',length=length(unique(data4A$species)))
...
test_statistics <- vector(mode='numeric',length=length(unique(data4A$species)))
p_values <- vector(mode='numeric',length=length(unique(data4A$species)))
...
#compare outputs
CV_vector <- vector(mode='numeric',length=length(unique(data4A$species)))
